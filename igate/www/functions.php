<?php
/**
 * functions.php — K6DRK iGate v5.2 shared utility functions
 *
...
function report_hostname() { // reports hostname
	if (str_contains(PHP_OS, 'WIN')) {
		return ("No idea. This is Windows!");
...
//		$direwolfsts = shell_exec("su -m pi -c 'whoami'");
//		if(str_contains($direwolfsts, "DIREWOLF")) {
//
// We don't run direwolf as a daemon -- we use systemd -- so the original code doesn't work.
// The web server runs as used www-data, to it can't see the direwolf process running under user pi.
// This workaround tests the modified age of the direwolf.log file, which updates frequently.
//
		$direwolflogfile="/var/log/direwolf/console.log";
		$oldtime=30;
		$currenttime=time();
		$filetime=filemtime($direwolflogfile);
		$timediff=$currenttime-$filetime;
		if ($timediff <= $oldtime) {
			return "<span class='running'>Running ($timediff secs ago)</span>";
		} else {
			return "<span class='notrunning'>Not running ($timediff secs ago)</span>";
		}
	} else {
		return "Operating System not supported";
	}
