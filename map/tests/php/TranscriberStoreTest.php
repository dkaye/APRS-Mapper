...
class TranscriberStoreTest extends TestCase
{
    ...
    // ── which device may speak for which channel ──────────────────────────────
    //
    // A device reports about its own channels and no others. There was a calibration
    // section here until 2026-08-29 that leaned on the same rule; it went out with the
    // SDR, along with the tuner gain and software squelch it existed to record. The rule
    // did not go anywhere -- the level feed and the heartbeat are both checked by it.
     ...
     *  Save. Same reasoning as the heartbeat file, and the same separate file. */
