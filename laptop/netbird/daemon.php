...
while (true) {
    ...
    // ── Force poll from admin changes (bypasses repeat throttle) ────────────────
    $pollForceFile = __DIR__ . '/poll_force';
    if ($viewerActive && file_exists($pollForceFile)) {
        @unlink($pollForceFile);
        logMsg("Poll forced (admin change)");
        $lastSendTime = 0;
    }

    // ── Honour one-shot poll request from api.php (stale data on page load) ───
    // api.php writes poll_now when stats are older than repeat_seconds.
    // We consume the file immediately (prevents pile-up from multiple clients)
    // and only act on it if a viewer is present and the period has elapsed.
    $pollNowFile = __DIR__ . '/poll_now';
    if ($viewerActive && file_exists($pollNowFile)) {
        @unlink($pollNowFile);
        if ($now - $lastSendTime >= $config['repeat_seconds']) {
            logMsg("Immediate poll triggered (page load, stale data)");
            $lastSendTime = 0;
        }
    }

    // ── Single-device poll (toggle-on from admin UI) ───────────────────────────
    $pollSingleFile = __DIR__ . '/poll_single';
    if ($viewerActive && file_exists($pollSingleFile)) {
        $singleIp = trim(@file_get_contents($pollSingleFile));
        @unlink($pollSingleFile);
        if (filter_var($singleIp, FILTER_VALIDATE_IP)) {
            $rawDevs   = loadDevices($config['yaml_file']);
            $singleDev = null;
            foreach ($rawDevs as $rd) {
                if ($rd['ip'] === $singleIp) { $singleDev = $rd; break; }
            }
            if ($singleDev && !empty($singleDev['enabled'])) {
                $t = time();
                if (!isset($stats[$singleIp])) {
                    $stats[$singleIp] = [
                        'ip'            => $singleIp,
                        'hostname'      => $singleDev['host'],
                        'name'          => $singleDev['name'],
                ...
                $r = @socket_sendto($sock, $config['request_message'],
                    strlen($config['request_message']), 0, $singleIp, $config['port']);
                $stats[$singleIp]['last_request'] = $t;
                if ($r === false) {
                    logMsg("poll_single failed → {$singleIp}: " . socket_strerror(socket_last_error($sock)));
                } else {
                    logMsg("poll_single → {$singleIp} ({$singleDev['host']} {$singleDev['name']})");
                }
                saveStats($stats, $config, $lastSendTime);
            }
        }
    ...
    if ($viewerActive && $now >= $lastSendTime + $config['repeat_seconds']) {
        ...
        foreach ($addresses as $ip => $info) {
            ...
            $stats[$ip]['last_request'] = $now;  // mark attempted regardless of send result
            if ($r === false) {
                $e = socket_last_error($sock);
