<?php
session_start();
if (empty($_SESSION['stats_auth'])) { header('Location: index.php'); exit; }
...
<title>MARS APRS Status — Admin</title>
<style>
...
.btn-ssh{background:#f3f4f6;border:1px solid #d1d5db;color:#374151;padding:3px 10px;border-radius:4px;cursor:pointer;font-size:12px;white-space:nowrap}
.btn-ssh:not(:disabled):hover{background:#e5e7eb}
.btn-ssh:disabled{opacity:.35;cursor:default}
...
<header>
  <h1>MARS APRS Status — Admin</h1>
  <button class="btn-add" onclick="openModal(null)">+ Add Device</button>
  <a href="index.php" class="hdr-btn">← Stats</a>
  <a href="/aprs/userguide.php?back=/aprs/status/admin.php#device-status-monitor" class="hdr-btn">User Guide</a>
</header>
...
function renderTable() {
    ...
    groupOrder.forEach(g => {
        if (g) html += `<tr class="group-row"><td colspan="6">${esc(g)}</td></tr>`;
        groupMap.get(g).forEach(d => {
            const safeD = JSON.stringify(d).replace(/</g, '\\u003c').replace(/'/g, '\\u0027');
            let sc, sl;
            if (!d.enabled) { sc = 'disabled'; sl = 'Disabled'; }
            else if (d.online) { sc = 'online'; sl = 'Online'; }
            ...
            html += `<tr class="device-row${d.enabled ? '' : ' disabled'}" data-ip="${esc(d.ip)}">
              ...
              <td class="action-cell"><div class="action-btns">
                <button class="btn-ssh" ${d.online ? '' : 'disabled'} onclick="openSSH('${esc(d.ip)}')">ssh</button>
                <button class="btn-edit" onclick='openModal(${safeD})'>Edit</button>
                <button class="btn-del"  onclick="deleteDevice('${esc(d.ip)}','${esc(d.name)}')">Del</button>
              </div></td>
            </tr>`;
        });
    });
    ...
    document.getElementById('tbody').innerHTML = html ||
        '<tr><td colspan="6" style="padding:40px;text-align:center;color:#9ca3af">No devices configured.</td></tr>';
}
        ...
        .then(resp => {
            ...
            if (resp.ok) {
                ...
                if (d) {
                    ...
                    renderTable();
                    try { const bc = new BroadcastChannel('aprs_status'); bc.postMessage({type:'toggle'}); bc.close(); } catch(e) {}
                }
            } else {
...
function deleteDevice(ip, name) {
    if (!confirm(`Delete "${name}" (${ip})?`)) return;
    const fd = new FormData();
